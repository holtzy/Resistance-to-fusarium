	#--------------------------------------------------------------------------------------------------
	#   
	#		ANALYSE des données de FUSARIOSE 2015. Partie Création carto
	#
	#					Script réalisé par 
...
# -------  QUE FAIT CE SCRIPT ???
	...
	# Ce script traite la partie création de la carte génétique. On part d'un fichier de génotypage pour arriver à une carte génétique finale.
		  ...
		  #
          #  4. Ordonnancement des marqueurs
...
for i in 1B 2A 2B 3A 3B 4B 5A 5B 6A 7A 7B ; do
Rscript /NAS/davem_data/EPO/SAUVEGARDE_DATA_YAN/programmes/return_LG_in_map.R carte_final.txt $i ;
done
